...
class Car:
    ...
    def cast(self, walls):
        ...
        angle_list = [0, 10, 20, 30, 45, 90, 135, 180]
        self.rays = []

        for angle in angle_list:
            if angle == 0:
                ray = Ray(self.x, self.y, self.soll_angle)
                self.rays.append(ray)
            elif angle == 180:
                ray = Ray(self.x, self.y, self.soll_angle + math.radians(180))
                self.rays.append(ray)
            else:
                ray1 = Ray(self.x, self.y, self.soll_angle - math.radians(angle))
                ray2 = Ray(self.x, self.y, self.soll_angle + math.radians(angle))
